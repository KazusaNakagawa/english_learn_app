...
### 2. Webhook URL を Secrets Manager に登録してデプロイ

Webhook URL はチャンネルへの投稿権限そのものなので、Lambda 環境変数ではなく
Secrets Manager に置きます (#182)。環境変数に入るのはシークレット名だけです。

```bash
aws secretsmanager create-secret \
  --name /englishlearn/poc/voicevox/slack-webhook-url \
  --description "Slack incoming webhook for VOICEVOX alerts (poc)" \
...
シークレットを作らない場合、Lambda はログのみ出力してエラーにはなりません（フォールバック済み）。
URL を差し替えるときは `put-secret-value` だけで済み、**再デプロイは不要**です
（反映は最大 5 分 — 温まったコンテナのキャッシュ TTL）。
